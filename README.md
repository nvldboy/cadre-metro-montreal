...
Le fichier privé [`pico/secrets.py`](pico/secrets.py) est ignoré par Git. Il ne
doit jamais être publié ou envoyé avec une clé réelle.
...
6. Regarder quelle DEL physique clignote.
7. Choisir la station placée devant elle, par exemple `Radisson` ou
   `Champ-de-Mars`.
8. Appuyer sur **Associer et passer à la suivante**.
9. Répéter jusqu’à ce que les 68 DEL soient associées.
10. Appuyer sur **Enregistrer et redémarrer**.
